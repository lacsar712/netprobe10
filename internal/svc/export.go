...
import (
	...
	"example.com/netprobe/internal/util/filedump"
	"example.com/netprobe/internal/util/jsonx"
)
...
func (c *Catalog) ExportJSON(ctx context.Context) ([]byte, error) {
	...
	exp := model.ExportFile{Name: "Netprobe TWAMP", Age: len(recs)}
	...
	snap := Snapshot{
		Title:   "Netprobe TWAMP",
		Count:   len(recs),
		Records: recs,
		Export:  decoded,
		Meta:    map[string]string{"app": "netprobe"},
	}
